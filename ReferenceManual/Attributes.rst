...
``NSManaged``
    ...
    Applying this attribute also applies the effect of the ``objc`` attribute.
...
Both the ``IBAction`` and ``IBOutlet`` attributes have the additional effect
of implicitly applying the ``objc`` attribute.
