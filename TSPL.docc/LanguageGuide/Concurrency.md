...
* Data race triangle
  ...
  - Overlapping
    Access is coming from different places in your code
    There's nothing serializing the access
...
* Resolve the risk of a data race by removing one of the three:

  - Make it a constant.
    Constants are immutable, so they can be shared.
    Especially useful for global variables.

  - Copy instead of sharing.
    Copies aren't shared, so they can be mutable.

  - Pass arguments as `transferring`,
    which prevents it being shared state
    TODO: Need an example to motivate this
    TODO: Should we explicitly contrast with `consuming`?

  - Use actors to prevent overlapping access
    and to prevent the mutable state from being shared.

  - Isolate to the Main actor for UI work.
    Code that's doing heavy computation should be elsewhere,
    but views and the like actually do belong here.

    Show the `Task { @MainActor in ... }` syntax

    Protocols that are `@MainActor` imply their conforming types are too

  - Use isolation domains to prove access doesn't overlap.

    Define isolation domain
    Summarize how the compiler reasons about them

    Show `MainActor.assumeIsolated` syntax,
    which will assert at run time if it's on the wrong actor,
    and `@preconcurrency` shorthand for it

* Alternate "fix it" outline:

  - **Make it immutable**
    Can you turn it into a constant?

  - **Don't share it**
    Can you copy, borrow, or consume it instead of sharing?
    Can you wrap it in an actor?

  - **Serialize access**
    Can you dispatch to an actor for access, like writing @MainActor on UI code?
