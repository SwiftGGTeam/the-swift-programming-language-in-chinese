...
.. _Functions_Autoclosure:

Autoclosure Function Type Parameters
------------------------------------

It is often useful to defer the evaluation of an expression passed
as function's parameter value until that value is needed.

Consider a hypothetical ``booleanOR(_:_:)`` function
that implements the logic of the Boolean ``OR`` operator (``||``),
returning ``true`` if either of the two arguments is ``true``:

.. testcode:: autoclosureFunction1

   -> func booleanOR(lhs: Bool, _ rhs: Bool) -> Bool {
          if lhs {
              return true
          } else if rhs {
              return true
...
When ``lhs`` is ``true, the value of ``rhs`` is not used.
As an optimization,
the evaluation of the expression passed as the ``rhs`` argument could be deferred
until only after it is determined that it is needed.
This ensures that no unnecessary computation is performed if it can be avoided, for example:

.. testcode:: autoclosureFunction1

   >> func expensiveFunctionThatReturnsBool() -> Bool {
   >>    return false
   >> }
   -> let truthValue = booleanOR(true, expensiveFunctionThatReturnsBool())
   << // truthValue : Bool = true

Changing the parameter type to a closure that returns the value of an expression
allows for the parameter value to be retrieved only as necessary by executing the closure.
However, this performance optimization comes at the cost of usability,
as now any call to the function requires the ``rhs`` argument to be a closure:

.. testcode:: autoclosureFunction2
   ...
   >>    return false
   >> }
   -> let truthValue = booleanOR(true, { return expensiveFunctionThatReturnsBool() } )
   << // truthValue : Bool = true

As a syntactic convenience, Swift provides the ``autoclosure`` attribute,
which allows an expression passed to a function type parameter
to be implicitly wrapped in a closure:

.. testcode:: autoclosureFunction3

   -> func booleanOR(lhs: Bool, @autoclosure _ rhs: () -> Bool) -> Bool {
          if lhs {
              return true
          } else {
              return rhs()
...
You can apply the ``autoclosure`` attribute to a parameter declaration
for a function type that has a parameter type of ``Void`` ---
such as ``() -> Bool``, as in the example above.
An autoclosure function captures an implicit closure over the specified expression,
instead of the expression itself.

.. _Functions_Autoclosure:

Escaping Autoclosure Functions
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

By default, the ``autoclosure`` attribute also implies the ``noescape`` attribute,
which denotes that a parameter will not be stored for later execution.
This prevents any references captured by the autoclosure function parameter
to outlive the lifetime of the function call.

You can specify that an autoclosure function should escape,
pass the ``escaping`` argument to the ``autoclosure`` attribute:

.. testcode:: functionLazyAssertion

   -> var lazyAssertions: [Void -> Void] = []
   << // lazyAssertions : [Void -> Void] = []
   -> func lazyAssertion(@autoclosure(escaping) condition: () -> Bool, message: String = "") {
         lazyAssertions.append({ if !condition() { print(message) } }) // escapes
      }
   ---
   -> let testNumber = 5
   -> lazyAssertion(testNumber % 2 == 0, message: "testNumber isn't an even number")
   >> for assertion in lazyAssertions {
   >>     assertion()
   >> }
