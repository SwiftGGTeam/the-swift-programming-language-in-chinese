...
如果您在访问其包含的值后不需要引用原始、可选常量或变量，则可以对新常量或变量使用相同的名称：

```swift
let myNumber = Int(possibleNumber)
// 这里，myNumber是一个可选的整数
if let myNumber = myNumber {
    // 这里，myNumber是一个不可选的整数
    print("My number is \(myNumber)")
}
// 输出“My number is 123”
```

此代码首先检查 `myNumber` 是否包含值，就像上一个示例中的代码一样。如果 `myNumber` 有一个值，则名为 `myNumber` 的新常量的值将设置为该值。在 `if` 语句的正文中，写 `myNumber` 指的是那个新的非可选常量。在 `if` 语句开始之前和结束之后，写 `myNumber` 指的是可选的整数常量。

由于这种代码非常常见，您可以使用较短的拼写来解开可选值：只写您正在展开的常量或变量的名称。新的、未包装的常量或变量隐式使用与可选值相同的名称。
...
您可以使用带有可选绑定的常量和变量。如果您想在 `if` 语句的第一个分支中操作 `myNumber` 的值，您可以编写 `if var myNumber`，并且可选中包含的值将作为变量而不是常量提供。您在 `if` 语句正文中对 `myNumber` 所做的更改仅适用于该局部变量，不适用于您解包装的原始可选常量或变量。
