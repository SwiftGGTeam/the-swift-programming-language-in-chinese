...
如果您在访问可选变量包含的值后不再需要使用原先的可选变量，则可以在解包的时候使用与可选变量相同的变量名定义新的常量或变量：
...
此代码首先检查 `myNumber` 是否包含确切的值，就像上一个示例中的代码一样。如果 `myNumber` 存在确切的值，则将该值赋给名为 `myNumber` 的新常量。在 `if` 语句的正文中，`let` 后的 `myNumber` 为新的非可选常量。在 `if` 语句开始之前和结束之后，`myNumber` 代表原先的可选整数常量。

由于这种代码非常常见，您可以通过简写的方式直接解包：在 `if` 后只需写与可选值相同名称的新常量或变量。
```swift
if let myNumber {
    print("My number is \(myNumber)")
}
...
您可以使用带有可选绑定的常量和变量。如果您想在 `if` 语句的第一个分支中更改 `myNumber` 的值，您可以采用 `if var myNumber` 语句将 `myNumber` 作为变量而不是常量。您在 `if` 分支语句中对变量 `myNumber` 所做的更改仅适用于该局部变量，不适用于您解包的原始可选常量或变量。
