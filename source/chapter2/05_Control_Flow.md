...
- [For 循环](#for_loops)
    - [For-In](#for_in)
    - [For条件递增（for-condition-increment）](#for_condition_increment)
...
- [控制传递语句（Control Transfer Statements）](#control_transfer_statements)
    ...
    - [带标签的语句（Labeled Statements）](#labeled_statements)
...
字典元素的遍历顺序和插入顺序可能不同，字典的内容在内部是无序的，所以遍历元素时不能保证顺序。关于数组和字典，详情参见[集合类型章节](../chapter2/04_Collection_Types.html)。
