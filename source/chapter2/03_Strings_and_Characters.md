...
您可以在您的代码中包含一段预定义的字符串值作为字符串常量。
字符串常量是由双引号 ("") 包裹着的具有固定顺序的文本字符集。

字符串常量可以用于为常量和变量提供初始值。
...
> 注意：  
`someString`常量通过字符串常量进行初始化，Swift 因此推断该常量为`String`类型。
...
可以将空的字符串常量赋值给变量，也可以初始化一个新的`String`实例：

```swift
var emptyString = ""               // 空字符串常量
...
您插入的字符串常量的每一项都被包裹在以反斜线为前缀的圆括号中：
