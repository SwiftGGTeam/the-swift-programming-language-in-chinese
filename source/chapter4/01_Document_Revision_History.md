...
* 更新至 Swift 5.1。
* 在 [不透明类型](../chapter2/23_Opaque_Types.md) 篇章中新增了有关函数返回值遵循指定协议，而不需要提供指定返回类型的内容。
...
* 更新至 Swift 4.1。
* 在 [等价运算符](../chapter2/27_Advanced_Operators.md#equivalence-operators) 章节新增了有关等价运算符的合成实现的内容。
...
* 更新至 Swift 3.0.1。
* 更新 [自动引用计数](../chapter2/24_Automatic_Reference_Counting.md) 章节中有关 weak 和 unowned 引用的讨论。
...
* 更新 [高级操作符](../chapter2/27_Advanced_Operators.md) 篇章中有关操作符的讨论，现在你可以作为类型函数来实现，替代之前的全局函数实现方式。
* 在 [访问控制](../chapter2/26_Access_Control.md) 章节中新增有关对新的访问级别描述符 `open` 和 `fileprivate` 的内容。
...
* 在 [高级操作符](../chapter2/27_Advanced_Operators.md) 篇章中 [自定义中缀操作符的优先级](./chapter2/27_Advanced_Operators.md#precedence-and-associativity-for-custom-infix-operators) 章节和 [定义](../chapter3/06_Declarations.md) 篇章中 [优先级组声明](../chapter3/06_Declarations.md#precedence-group-declaration-modifiers) 章节中新增了有关操作符优先级组的内容。
...
* 更新 [弱引用](../chapter2/24_Automatic_Reference_Counting.md#weak-references) 章节中的图片用以更清楚的展示重新分配过程。
...
* 在 [访问控制](../chapter2/26_Access_Control.md) 篇章的 [单元测试 target 的访问级别](../chapter2/26_Access_Control.md#access-levels-for-unit-test-targets) 章节中新增了有关单元测试访问控制相关的内容。
...
* 在 [弱引用](../chapter2/24_Automatic_Reference_Counting.md#weak-references) 章节新增了一处说明，有关弱引用作为缓存所存在的不足。
...
* 在 [访问级别](../chapter2/26_Access_Control.md#access-levels) 章节新增了有关 `private` 作用域的相关内容说明。
* 在 [弱引用](../chapter2/24_Automatic_Reference_Counting.md#weak-references) 章节新增了有关弱应用在垃圾回收系统和 ARC 之间的区别的说明。
...
* 在 [溢出运算符](../chapter2/27_Advanced_Operators.md#overflow-operators) 一节中删除了溢出除运算符（`&/`）和求余溢出运算符（`&%`）。
...
* 更新 [值溢出](../chapter2/27_Advanced_Operators.md#value-overflow) 章节，对有符号整数和无符号整数的溢出行为进行了阐明。
* 更新 [协议声明](../chapter3/06_Declarations.md#protocol-declaration) 章节，对协议声明时的作用域和成员等内容进行了阐明。
* 更新 [捕获列表](../chapter2/24_Automatic_Reference_Counting.md#defining-a-capture-list) 章节，对于闭包捕获列表中的弱引用和无主引用的使用语法进行了阐明。
...
* 自定义运算符现在可以包含 `?` 字符，更新了 [运算符](../chapter3/02_Lexical_Structure.md#operator) 涉及改进后的规则的部分，并且在 [自定义运算符](../chapter2/27_Advanced_Operators.md#custom-operators) 章节中删除了重复的运算符有效字符集合。
...
* 中置（Infix）的 [运算符函数](../chapter2/27_Advanced_Operators.md#operator-functions) 不再需要 `@infix` 属性。
* [前置和后置运算符](../chapter2/27_Advanced_Operators.md#prefix-and-postfix-operators) 的 `@prefix` 和 `@postfix` 属性，已变更为 `prefix` 和 `postfix` 声明修饰符。
* 新增一处说明，在 Prefix 和 postfix 运算符被作用于同一个操作数时 [前置和后置运算符](../chapter2/27_Advanced_Operators.md#prefix-and-postfix-operators) 的执行顺序。
* [组合赋值运算符](../chapter2/27_Advanced_Operators.md#compound-assignment-operators) 的运算符函数不再使用 `@assignment` 属性来定义函数。
* 在定义 [自定义操作符](../chapter2/27_Advanced_Operators.md#custom-operators) 时，`修饰符（Modifiers）的出现顺序发生变化`。比如现在，你该编写 `prefix operator`， 而不是 `operator prefix`。
...
* 新增更多有关柯里化函数的内容。
* 新增 [权限控制](../chapter2/26_Access_Control.md) 篇章。
...
* 解释了哪些字符（集）可被用来定义 [自定义操作符](../chapter2/27_Advanced_Operators.md#custom-operators)。
